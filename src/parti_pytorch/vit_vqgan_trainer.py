...
class VQGanVAETrainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.ema_vae = EMA(self.vae, update_after_step = ema_update_after_step, update_every = ema_update_every)
        ...
        if len([*self.results_folder.glob('**/*')]) > 0:
    ...
    def train_step(self):
        ...
        if exists(self.vae.discr):
            ...
            print(f"[{cur_time}] {steps}: vae loss: {logs['loss']} - discr loss: {logs['discr_loss']}")
